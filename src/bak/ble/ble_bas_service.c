...
APP_TIMER_DEF(bas_svc_timer);

static void bas_svc_upd_handler(void * p_context)
{
    ret_code_t err_code;
    uint8_t battery_level = kb_voltage_percentage();
...
void battery_service_init(void)
{
    ...
    err_code = app_timer_create(&bas_svc_timer,
                                APP_TIMER_MODE_REPEATED,
                                bas_svc_upd_handler);
    APP_ERROR_CHECK(err_code);
    err_code = app_timer_start(bas_svc_timer, BATTERY_LEVEL_MEAS_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
}